...
public class DisponibilidadService {
	...
	// 🔥 CONSTRUCTOR
	public DisponibilidadService(DisponibilidadRepository dispoRepo, medicoRepository medicoRepo,
	...
	public void ValidarTurnosConMedico(Long medicoId, LocalDate fecha, LocalTime hora, Long orgId) {
		...
		disponibilidades = disponibilidades.stream().filter(d -> d.getMedico().getOrganizacion().getId().equals(orgId))
				.toList();
	...
	public List<DisponibilidadResponseDto> listarDisponibilidad(Long medicoId, Long orgId) {
		...
		return lista.stream().filter(d -> d.getMedico().getOrganizacion().getId().equals(orgId))
				.map(d -> new DisponibilidadResponseDto(d.getId(), d.getMedico().getNombre(), d.getDiaSemana(),
						d.getHoraInicio() + " - " + d.getHoraFinal(), d.getDuracionMinutos()))
				.toList();
	}
	...
	public DisponibilidadResponseDto crearDisponibilidad(DisponibilidadRequestDto dto, Long orgId) {
		...
		boolean solapado = existentes.stream().anyMatch(d -> d.getDiaSemana() == dto.getDiaSemana()
				&& dto.getHoraInicio().isBefore(d.getHoraFinal()) && dto.getHoraFin().isAfter(d.getHoraInicio()));

		if (solapado) {
			throw new RuntimeException("Ya existe una disponibilidad en ese rango horario");
		}

		validarSolapamiento(medico.getId(), dto.getDiaSemana(), dto.getHoraInicio(), dto.getHoraFin());
		...
		return new DisponibilidadResponseDto(guardado.getId(), medico.getNombre(), guardado.getDiaSemana(),
				guardado.getHoraInicio() + " - " + guardado.getHoraFinal(), guardado.getDuracionMinutos());
	}
	...
	public void eliminarDisponibilidad(Long id, Long orgId) {
		...
		Disponibilidad d = dispoRepo.findById(id).orElseThrow(() -> new RuntimeException("No encontrada"));
